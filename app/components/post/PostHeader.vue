...
<template>
    <!-- 💩 Trình duyệt Quark, trên máy tính chỉ có IE không hỗ trợ :has() -->
    <div class="post-header" :class="{ 'has-cover': image, 'text-revert': cover_revert }">
        ...
        <div class="post-nav">
            ...
            <div v-if="!hideInfo" class="post-info">
                <time v-if="date" v-tip="`Tạo vào ${getLocaleDatetime(props.date)}`" :datetime="getIsoDatetime(date)">
                    <Icon name="ph:calendar-dots-bold" /> {{ publishedLabel }}
                </time>
                <time
                    v-if="isTimeDiffSignificant(date, updated, .999)"
                    v-tip="`Chỉnh sửa vào ${getLocaleDatetime(props.updated)}`"
                ...
                <span class="wordcount">
                    <Icon name="ph:paragraph-bold" /> {{ formatNumber(readingTime?.words) }} từ
                </span>
            </div>
        </div>
